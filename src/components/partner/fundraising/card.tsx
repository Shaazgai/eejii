    ...
    <Paper display={'flex'} withBorder radius="md" className={cardClasses.card}>
      <SimpleGrid w={'100%'} cols={{ base: 1, md: 2, lg: 2 }}>
        <Group miw={400}>
          ...
          >
            <Text fz="24">
              {format(fundraising?.createdAt as unknown as Date, 'MMM')}
            </Text>
            <Text fz="38">
              {format(fundraising?.createdAt as unknown as Date, 'd')}
            </Text>
            <Text c={'dimmed'}>
              {format(fundraising?.createdAt as unknown as Date, 'yyyy')}
            </Text>
          </Flex>
          <Flex justify={'around'} direction={'column'}>
            ...
            <Text size="sm" mt="sm" c="dimmed">
              {format(
                fundraising?.startTime as unknown as Date,
                'E MMMM-d, yyy'
              )}
              {format(fundraising?.endTime as unknown as Date, 'E MMMM-d, yyy')}
            </Text>
          </Flex>
        </Group>
        <Flex
