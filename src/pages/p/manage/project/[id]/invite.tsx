...
const Invite = () => {
  ...
  const { data: users, isLoading: isUsersLoading } =
    api.project.findUsersToInvite.useQuery({
      projectId: projectId,
      userType: userType as string,
    });
  const { mutate: invite, isLoading: isInviteLoading } =
    api.projectUser.inviteToProject.useMutation({
      onSuccess: () => {
        context.project.findUsersToInvite.invalidate();
      },
    });
    ...
    <PartnerLayout>
      <div className="space-y-5 p-24">
        ...
        <div className="flex justify-end">
          <Button
            onClick={() => {
              router.push(`/p/manage/${projectId}`);
            }}
          >
            Skip
          </Button>
